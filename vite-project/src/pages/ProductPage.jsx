...
function ProductPage({ onAddToCart, isLoggedIn }) {
  const { id } = useParams()
  ...
  const allProducts = [
    ...
    {
      id: 4,
      name: 'Beaded Traditional Necklace',
  ...
  useEffect(() => {
    ...
    setIsWishlisted(wishlisted.includes(parseInt(id)))
  }, [id])
  ...
  const handleWishlist = () => {
    const wishlisted = JSON.parse(localStorage.getItem('wishlisted') || '[]')
    if (isWishlisted) {
      const updated = wishlisted.filter(id => id !== product.id)
      localStorage.setItem('wishlisted', JSON.stringify(updated))
    } else {
      localStorage.setItem('wishlisted', JSON.stringify([...wishlisted, product.id]))
    }
